...
- **Per-project issue numbers** — every issue gets a human-friendly `#N` that is sequential within its project (each project independently starts at `#1`), shown on the board card, the Issues table, and the issue modal. Allocation is atomic and **deadlock-free**: a `BEFORE INSERT` trigger bumps a single per-project counter row via `INSERT … ON CONFLICT … RETURNING`, and existing data is backfilled idempotently on schema apply
