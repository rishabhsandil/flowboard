...
### Real-time board updates (SignalR)
- [x] `BoardHub` at `/hubs/board` (`Microsoft.AspNetCore.SignalR`) — `[Authorize]`-gated, per-project groups (`project:{projectId}`), JWT bearer token supplied via `?access_token=…` for the WebSocket transport (lifted into `ctx.Token` in the `JwtBearerEvents.OnMessageReceived` hook)
- [x] Hub rejects with `HubException` if the caller is not a member of the requested project; non-members see a closed connection rather than silent fan-out
- [x] `IBoardEventPublisher` (`SignalRBoardEventPublisher`) wraps `IHubContext<BoardHub>`; swallows transient send failures so a disconnecting client cannot poison the underlying write
- [x] `ActivityLogger.LogAsync` calls `_publisher.PublishAsync(projectId, type, payload)` after every audited insert — activities are the canonical event log, so every existing controller gets real-time fan-out for free
- [x] Audit gap-fill: `column_created` / `column_updated` / `column_deleted` / `columns_reordered` now log activity (and therefore publish), so the board stays in sync when a teammate edits the column layout
- [x] CORS hardened for SignalR: `AllowCredentials()` plus the `x-signalr-user-agent` header explicitly allow-listed (no wildcards — keeps coding-standard A11 intact)
- [x] React `useBoardSocket(projectId)` hook — `@microsoft/signalr` connection keyed by project, `accessTokenFactory` reads live from `useAuthStore` so token rotation works without a manual reconnect, `withAutomaticReconnect()` + blanket invalidation on reconnect as a missed-event safety net
- [x] Mounted from `ProjectLayout` so the connection tears down on project switch
- [x] Event → cache map (`cacheEffectsFor`) covers `issue.*`, `column.*`, `comment.*` / `mention`, `label.*`, `dependency.*`, `member.*`; unknown types fall back to a project-wide refresh so backend additions never go silently undelivered
- [x] Tests: `SignalRFlowTests` (auth handshake, project-group isolation, end-to-end fan-out from controller writes) + Playwright two-browser-context e2e (`realtime.spec.ts`) for both create and delete
